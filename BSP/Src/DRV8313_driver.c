/**
 * @file					DRV8313_driver.c
 * @author 					可以航行
 * @version 				0.3
 * @date 					2026/9/1
 * @brief 					DRV8313驱动代码(注入组采样电流 + 普通组DMA采样母线电压)
 *
...
void vDRV8313DeviceInit(stDRV8313StaticParameTdf *pstInit, enumDRV8313DeviceNumTdf emDeviceNum)
{
	...
	pstDev->DRV8313DynamicParame.u8OcFlag     = 0;
	pstDev->DRV8313DynamicParame.u8InjDone    = 0;
	pstDev->DRV8313DynamicParame.u8CalState   = 0;
...
void vDRV8313ReadCurrent(enumDRV8313DeviceNumTdf emDeviceNum)
{
	...
	for (u8Phase = 0; u8Phase < 3; u8Phase++)
	{
		...
		{
			pstDev->DRV8313DynamicParame.u8OcFlag = 1;
		}
	}
...
void vDRV8313CalibrateOffset(enumDRV8313DeviceNumTdf emDeviceNum)
{
	...
	if (pstDev->DRV8313DynamicParame.u8CalState >= 2)
	{
		return;
	}
	...
	if (pstDev->DRV8313DynamicParame.u32CalCount < DRV8313_CAL_NUM)
	{
		...
		pstDev->DRV8313DynamicParame.u8CalState = 1;
	...
	{
		/* 采样次数到,校准完成 */
		pstDev->DRV8313DynamicParame.u8CalState  = 2;
		pstDev->DRV8313DynamicParame.u32CalCount = 0;
	}
}
/*
...
{
	return arrystDRV8313Deviceparam[emDeviceNum].DRV8313DynamicParame.u8CalState;
}
/*
...
{
	stDRV8313DeviceParameTdf 	*pstDev = &arrystDRV8313Deviceparam[emDRV8313DeviceNum0];

	if (hadc != pstDev->DRV8313StaticParame.pstADCHandle)
	{
		return;
	}
	pstDev->DRV8313DynamicParame.u8InjDone = 1;
	vDRV8313ReadCurrent(emDRV8313DeviceNum0);
}
/*
 * 	@brief 										启动普通组DMA转换
 * 	@param		emDeviceNum		 	DRV8313设备号
...
{
	return arrystDRV8313Deviceparam[emDeviceNum].DRV8313DynamicParame.u8OcFlag;
}
