/**
  ...
  * 统一修改。电流环代码位于 App/Src/app_motor.c,由 PWM 同步注入组
...
/* 电流环默认 q 轴电流参考(A),上电/切闭环时的初始给定,便于测试 */
#define DEFAULT_CUR_LOOP_IQ_REF		0.5f

/* ==================== 上电调机(转子对齐 + 方向确认) ==================== */
/* 首次以编码器来源启动自动执行(用电流环 I-f 方式,限流防过流):
 *   ① 对齐: 注入恒定小 Id 电流沿 θ=0 锁转子,等待后捕获编码器零位;
 *   ② 方向确认: 正向缓慢旋转电流矢量拖转子,观察编码器电角度是否随动,
 *      若反向则自动置编码器方向标志 -1,再对齐;完成进入电流环闭环。 */
#define COMM_ALIGN_ID_A				1.0f	/* 对齐/方向确认注入 d 轴电流(A),I-f 限流 */
#define COMM_ALIGN_LOCK_MS			800		/* 对齐锁定时间(ms) */
#define COMM_VERIFY_FREQ_HZ			4.0f	/* 方向确认电频率(Hz,正向) */
#define COMM_VERIFY_MS				600		/* 方向确认观察窗口(ms) */
