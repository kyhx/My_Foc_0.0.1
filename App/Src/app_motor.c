...
/**
 * @brief 								电机控制应用初始化
 * @note									初始化电流环 PI、电流参考、开环电压与模式(默认开环),
...
void vAppMotorInit(void)
{
	...
	s_u8Run  = 0;
	s_emMode = emAppMotorMode_Current;	/* 默认电流环(FOC闭环)启动 */

	s_fOlVd  = 0.0f;
	s_fOlVq  = 0.0f;

	s_fIdRef = 0.0f;
	s_fIqRef = DEFAULT_CUR_LOOP_IQ_REF;		/* 电流环默认 q 轴电流(A),便于测试 */
	s_fId    = 0.0f;
	s_fIq    = 0.0f;
	s_fVd    = 0.0f;
	s_fVq    = 0.0f;
...
{
	if (s_emMode == emAppMotorMode_OpenLoop)
	{
